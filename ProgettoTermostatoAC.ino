...
class DataHolder
{
  public:
    ...
    //A/C
    int ac_switchManOnOffCool;
    int ac_switchManOnOffHeat;
    int ac_switchManOnOffDry;
...
class RemoteNodeInfo
{
  public:
    ...
    //for A/C only
    String ac_setpoint_temperature_hot;
    String ac_setpoint_temperature_cold;
    String ac_setpoint_humidity_dry;
    String ac_power_state;
    String ac_fan_speed;
    String ac_mode;
    String ac_swing_vertical;    
};
...
void onMessage(WebsocketsClient& client, WebsocketsMessage message) {
  ...
  if(msg[0]=='!')
  {
    ...
    */
    ...
    if (num_split<3)
      return;
    String client_node_id = strs[0];//remove "!"
    String client_node_temperature = strs[1];
    String client_node_humidity = strs[2];
    ...
    for (auto& node : vecRemoteNodes) {
        if(node.node_id == client_node_id)
        {
          ...
          {
            //Data from A/C
            node.ac_power_state = strs[3];
            node.ac_fan_speed = strs[4];
            node.ac_mode = strs[5];
            node.ac_setpoint_temperature_hot = strs[6];
            node.ac_setpoint_temperature_cold = strs[7];
            node.ac_setpoint_humidity_dry = strs[8];
            node.ac_swing_vertical = strs[9];
          }
          client_present=true;
          break;
        }
    ...
    if(!client_present)
    {
      ...
      {
        //Data from A/C
        nr.ac_power_state = strs[3];
        nr.ac_fan_speed = strs[4];
        nr.ac_mode = strs[5];
        nr.ac_setpoint_temperature_hot = strs[6];
        nr.ac_setpoint_temperature_cold = strs[7];
        nr.ac_setpoint_humidity_dry = strs[8];
        nr.ac_swing_vertical = strs[9];
      }
    ...
    if(client_node_id == "3")
    {
      //send trought websocket command dato to node 3 with A/C commands
      String acmode;
      
      if(g_DataHolder.ac_switchManOnOffHeat==1)
      {
        acmode="Heat";
        g_DataHolder.ac_OnOffCmd=true;
      }
      else if(g_DataHolder.ac_switchManOnOffCool==1)
      {
        acmode="Cool";
        g_DataHolder.ac_OnOffCmd=true;
      }
      else if(g_DataHolder.ac_switchManOnOffDry==1)
      {
         acmode="Dry";
         g_DataHolder.ac_OnOffCmd=true;
      }
      ...
      {
         acmode="Dry";
         g_DataHolder.ac_OnOffCmd=false;
      }
      int cmd_sum=g_DataHolder.ac_switchManOnOffHeat+g_DataHolder.ac_switchManOnOffCool+g_DataHolder.ac_switchManOnOffDry;
      if(cmd_sum>1 || cmd_sum<=0)
      {
        //only one command available
        //if multiple commands activated we turn-off the A/C
        acmode="Dry";
        g_DataHolder.ac_OnOffCmd=false;
      }
      ...
      char msg[80];
      sprintf(msg,"!%d,%s,%s,%s,%d,%d", 3, acmode,String(g_DataHolder.ac_setpoint_temperature_hot,1),
      String(g_DataHolder.ac_setpoint_temperature_cold,1),(int)g_DataHolder.ac_OnOffCmd, g_DataHolder.ac_fanspeed);
      client.send(msg);          
...
bool read_from_nextion_core_logic_boiler()
{
  if(timer_sampling_read_from_nextion.getET()>=sampling_time_read_from_nextion)
  {
   ...
   // Serial.println(set_temp);
    ...
    /* READ A/C CONTROL DATA */
    //switch on-off heat
    g_DataHolder.ac_switchManOnOffHeat=myNex.readNumber("page4.ac_cmdh.val");
    if (g_DataHolder.ac_switchManOnOffHeat==777777)
    {
      return false;
    }
    set_temp =myNex.readNumber("page4.ac_sph.val");
    if (set_temp==777777)
    {
      return false;
    }
    g_DataHolder.ac_setpoint_temperature_hot = set_temp*0.1;
    //switch on-off cold
    g_DataHolder.ac_switchManOnOffCool=myNex.readNumber("page4.ac_cmdc.val");
    if (g_DataHolder.ac_switchManOnOffCool==777777)
    {
      return false;
    }
    set_temp =myNex.readNumber("page4.ac_spc.val");
    if (set_temp==777777)
    {
      return false;
    }
    g_DataHolder.ac_setpoint_temperature_cold = set_temp*0.1;
    //switch on-off dry
    g_DataHolder.ac_switchManOnOffDry=myNex.readNumber("page4.ac_cmdd.val");
    if (g_DataHolder.ac_switchManOnOffDry==777777)
    {
      return false;
    }
    /*
    set_temp =myNex.readNumber("page4.ac_spd.val");
    if (set_temp==777777)
    {
      return false;
    }
    g_DataHolder.ac_setpoint_humidity_dry = set_temp*0.1;
    */

    g_DataHolder.ac_fanspeed=myNex.readNumber("page4.ac_cmdfs.val");
    if (g_DataHolder.ac_fanspeed==777777)
    {
      return false;
    }
    if(g_DataHolder.ac_fanspeed>3)
      g_DataHolder.ac_fanspeed=3;
    else if(g_DataHolder.ac_fanspeed<0)
      g_DataHolder.ac_fanspeed=0;
...
void write_to_nextion()
{
  if(timer_sampling_write_to_nextion.getET()>=sampling_time_write_to_nextion)
  {
    ...
    for (auto& node : vecRemoteNodes) {
        ...
        {
          //bar info on page4
          myNex.writeStr("page4.node"+node.node_id+"_temp.txt",node.node_temperature);
          myNex.writeStr("page4.node"+node.node_id+"_hum.txt",node.node_humidity);
          //page4 A/C control
          myNex.writeStr("page4.node"+node.node_id+"_state.txt",node.ac_power_state);
          myNex.writeStr("page4.node"+node.node_id+"_fs.txt",node.ac_fan_speed);          
          myNex.writeStr("page4.node"+node.node_id+"_mode.txt",node.ac_mode);
          myNex.writeStr("page4.node"+node.node_id+"_sph.txt",node.ac_setpoint_temperature_hot);
          myNex.writeStr("page4.node"+node.node_id+"_spc.txt",node.ac_setpoint_temperature_cold);
          //myNex.writeStr("page4.node"+node.node_id+"_spd.txt",node.ac_setpoint_humidity_dry);
          myNex.writeStr("page4.node"+node.node_id+"_swing.txt",node.ac_swing_vertical);
        }
    }
    //central station
